...
export default function StaffProjects() {
  ...
  const fetchProjects = useCallback(async () => {
    ...
    let query = supabase
      .from("projects")
      .select("id, title, description, status, location, target_date, updated_at")
    ...
    <PortalLayout variant="staff">
      ...
      {loading ? (
        ...
        <div className="rounded-xl border border-portal-border bg-portal-surface flex flex-col items-center justify-center py-16 text-portal-text-muted">
          ...
          <p className="font-medium text-portal-text">No assigned projects</p>
          <p className="text-sm mt-1">An admin will assign projects to you.</p>
        </div>
      ) : (
